...
router.post('/logout', authRequired, async (req, res, next) => {
	try {
		...
		if (!token) {
			...
			throw new AppError('AUTHORIZATION TOKEN MISSING!', 401, 'AUTH', 'ERR_TOKEN_MISSING');
		}
		...
		return next(new AppError('UNEXPECTED ERROR IN LOGOUT FUNCTION!', 500, 'LOGOUT', 'ERR_LOGOUT_FAILED'));
	}
});
